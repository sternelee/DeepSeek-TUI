...
pub fn auto_model_heuristic_with_bias(
    ...
    let borderline_pro_keywords: &[&str] = &["implement", "analyze"];

    let pro_match = strong_pro_keywords.iter().any(|kw| lower.contains(kw))
        || (!cost_saving && borderline_pro_keywords.iter().any(|kw| lower.contains(kw)));
    if pro_match {
